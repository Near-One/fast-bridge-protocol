...
describe("Spectre Bridge", () => {
    ...
    before(async () => {
        ...
        await proxy.connect(owner).grantRole(await proxy.PAUSABLE_ADMIN_ROLE(), pausableAdmin.address); // already there in init
    ...
    it("Should deploy Bridge with tokens and whitelist states", async () => {
        ...
        proxyV1 = await upgrades.deployProxy(bridgeV1, [tokensAddresses, tokensWhitelistStatuses], {
    ...
    it("Should upgrade Bridge ", async () => {
        ...
        const proxyV2 = await upgrades.upgradeProxy("0xc3023a2c9f7B92d1dd19F488AF6Ee107a78Df9DB", bridgeV1, {
            unsafeAllow: ["delegatecall"]
        });
    });
